...
this.ecmreviewForm  = this.fb.group({pk:[null],
reviewid: [null],
productid: [null],
customerid: [null],
reviewdate: [null],
...
reviewdate: this.ngbDateParserFormatter.parse(new Date().toISOString()),
});
}
else
{
let obj =  this.crmcustomermasterservice.ecmreviews.filter(x => (x as any).pkcol == ppk)[0];
this.ecmreviewForm.patchValue({
reviewid:  obj.reviewid,
productid:  obj.productid,
customerid:  obj.customerid,
reviewdate: this.ngbDateParserFormatter.parse(obj.reviewdate as any),
...
if(key!='visiblelist' && key!='hidelist'){
    if (this.ecmreviewForm.controls[key] != null)
    {
        obj[key] = this.ecmreviewForm.controls[key].value;
    }
}
}
}
obj.reviewdate=this.ngbDateParserFormatter.format(this.ecmreviewForm.get('reviewdate').value);
console.log(obj);
...
}
{
        if(this.ecmreviewForm.controls[key]!=null)
{
this.ecmreviewForm.patchValue(json);
         if(bdisable)this.ecmreviewForm.controls[key].disable({onlySelf: true});
}
      }
      }
      }
