...
@Component({
    ...
    templateUrl: './hmsreceipts.component.html',
    styles: [],
    providers: [KeyboardShortcutsService]
})
export class hmsreceiptsComponent implements OnInit {
    ...
    constructor(
        ...
        this.hmsreceiptsForm = this.fb.group({
            receiptid: [null],
            patientid: [null],
            doctorid: [null],
            doctoriddesc: [null],
    ...
    get f() { return this.hmsreceiptsForm.controls; }
    ...
    async ngOnInit() {
        ...
        if (ppk == null) {
            this.hmsreceiptsForm.patchValue({
    ...
    onSubmitData(bclear:any) {
        ...
        if (!this.hmsreceiptsForm.valid) {
            this.toastr.addSingle("error", "", "Enter the required fields");
            return;
        }
        var obj = this.hmsreceiptsForm!.value;
        obj.receiptdate = this.ngbDateParserFormatter.format(this.hmsreceiptsForm.get('receiptdate')!.value);
        obj.receipttime = (this.hmsreceiptsForm.get('receipttime')!.value == null ? 0 : this.hmsreceiptsForm.get('receipttime')!.value.hour) + ':' + (this.hmsreceiptsForm.get('receipttime')!.value == null ? 0 : this.hmsreceiptsForm.get('receipttime')!.value.minute);
        console.log(obj);
    ...
    doctoridonChange(evt:any) {
        let e = evt!.value;
        this.hmsreceiptsForm.patchValue({ doctoriddesc: evt.options[evt.options.selectedIndex].text });
    }
    paymentcategoryonChange(evt:any) {
        let e = evt!.value;
        this.hmsreceiptsForm.patchValue({ paymentcategorydesc: evt.options[evt.options.selectedIndex].text });
    }
    paymentmodeonChange(evt:any) {
        let e = evt!.value;
        this.hmsreceiptsForm.patchValue({ paymentmodedesc: evt.options[evt.options.selectedIndex].text });
    }
