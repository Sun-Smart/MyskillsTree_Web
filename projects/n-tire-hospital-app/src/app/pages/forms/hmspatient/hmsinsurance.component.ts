...
@Component({
    selector: 'app-hmsinsurances',
    templateUrl: './hmsinsurance.component.html',
    styles: [],
    providers: [KeyboardShortcutsService]
})
export class hmsinsuranceComponent implements OnInit {
    ...
    constructor(
        ...
        this.hmsinsuranceForm = this.fb.group({
            insuranceid: [null],
            patientid: [null],
            insurancedperson: [null],
            relationship: [null],
    ...
    get f() { return this.hmsinsuranceForm.controls; }
    ...
    async ngOnInit() {
        ...
        if (ppk == null) {
            this.hmsinsuranceForm.patchValue({
            });
        }
        else {
            let obj = this.hmspatientservice.hmsinsurances.filter(x => x.insuranceid == ppk)[0];
            this.hmsinsuranceForm.patchValue({
                insuranceid: obj.insuranceid,
                patientid: obj.patientid,
                insurancedperson: obj.insurancedperson,
    ...
    onSubmitData(bclear:any) {
        ...
        if (!this.hmsinsuranceForm.valid) {
            this.toastr.addSingle("error", "", "Enter the required fields");
            return;
        }
        var obj = this.hmsinsuranceForm!.value;
        console.log(obj);
    ...
    relationshiponChange(evt:any) {
        let e = evt!.value;
        this.hmsinsuranceForm.patchValue({ relationshipdesc: evt.options[evt.options.selectedIndex].text });
    }
