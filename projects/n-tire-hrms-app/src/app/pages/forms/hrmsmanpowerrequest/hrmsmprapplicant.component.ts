...
import { hrmsapplicantmaster, IhrmsapplicantmasterResponse } from '../../../../../../n-tire-hrms-app/src/app/model/hrmsapplicantmaster.model';
...
export class hrmsmprapplicantComponent implements OnInit {
  ...
  applicantidoptionsEvent: EventEmitter<any> = new EventEmitter<any>();
  applicantid_hrmsapplicantmastersForm: FormGroup;
  applicantid_hrmsapplicantmastersoptions: any;
  applicantid_hrmsapplicantmastersformatter: any;
  ...
  async ngOnInit() {
    ...
    if (this.pkcol == null) {
      ...
      let obj = this.hrmsmanpowerrequestservice.hrmsmprapplicants.filter(x => (x as any).pkcol == ppk)[0];
      this.hrmsmprapplicantForm.patchValue({
        mprid: obj.mprid,
        applicantid: obj.applicantid,
        applicantiddesc: obj.applicantiddesc,
    ...
    this.hrmsapplicantmasterservice.gethrmsapplicantmastersList().then((res:any) => {
      this.applicantidList = res as hrmsapplicantmaster[];
      if (this.formdata && this.formdata.hrmsmprapplicant && this.formdata.hrmsmprapplicant.applicantid) {
        this.applicantidoptionsEvent.emit(this.applicantidList);
        this.hrmsmprapplicantForm.patchValue({
          applicantid: this.formdata.hrmsmprapplicant.applicantid,
          applicantiddesc: this.formdata.hrmsmprapplicant.applicantiddesc,
        });
      }
    }
    );
  ...
  AddOrEditapplicantid(applicantid) {
    ...
    this.hrmsapplicantmasterservice.gethrmsapplicantmastersList().then((res:any) => this.applicantidList = res as hrmsapplicantmaster[]);
    });*/
  }
