...
bfilterPopulatebosubcategorymasters:boolean=false;
 bosubcategorymasterForm: FormGroup;
...
this.bosubcategorymasterForm  = this.fb.group({pk:[null],subcategoryid: [null],
categoryid: [null],
...
get f() { return this.bosubcategorymasterForm.controls; }
...
debugger;
if (this.bosubcategorymasterForm.dirty && this.bosubcategorymasterForm.touched ) {
...
//this.sharedService.alert(bosubcategorymasterid);
...
this.bosubcategorymasterForm.markAsUntouched();
this.bosubcategorymasterForm.markAsPristine();
}



resetForm() {
if (this.bosubcategorymasterForm != null)
this.bosubcategorymasterForm.reset();
this.bosubcategorymasterForm.patchValue({
});
this.PopulateFromMainScreen(this.data,false);
this.PopulateFromMainScreen(this.dynamicconfig.data,true);
}
    ...
    onDelete() {
        let subcategoryid = this.bosubcategorymasterForm.get('subcategoryid').value;
        if(subcategoryid!=null)
        {
        if (confirm('Are you sure to delete this record ?')) {
            this.bosubcategorymasterservice.deletebosubcategorymaster(subcategoryid).then(res =>
                {
                this.resetForm();
                }
            ).catch((err) => {console.log(err);});
    ...
    if(this.bosubcategorymasterservice.formData.subcategoryid!=null)this.bosubcategorymasterservice.formData.subcategoryid=null;
    }
...
{
this.bosubcategorymasterForm.patchValue({[key]:  mainscreendata[key] } );
}
        else
{
this.bosubcategorymasterForm.patchValue({[key]:  mainscreendata[key] } );
...
{
{
         if(bdisable && this.bosubcategorymasterForm.controls[key]!=undefined)this.bosubcategorymasterForm.controls[key].disable({onlySelf: true});
}
      }
      }
      }
...
this.bosubcategorymasterservice.getbosubcategorymastersByEID(pkcol).then(res => {

this.bosubcategorymasterservice.formData=res;
...
getHtml(html:any)
{
  let ret = "";
  ret = html;
  for (let key in this.bosubcategorymasterForm.controls) {
    if (this.bosubcategorymasterForm.controls[key] != null) {
      ret = ret.replace(new RegExp('##' + key + '##', 'g'), this.bosubcategorymasterForm.controls[key].value);
    }
  }
  return ret;
}
...
if(!this.bosubcategorymasterForm.valid)
{
this.toastr.addSingle("error", "", "Enter the required fields");
return; 
}
var obj=this.bosubcategorymasterForm.value;
console.log(obj);
...
this.isSubmitted = true;
let strError="";
Object.keys(this.bosubcategorymasterForm.controls).forEach(key => {
    const controlErrors: ValidationErrors = this.bosubcategorymasterForm.get(key).errors;
...
this.bosubcategorymasterservice.formData=this.bosubcategorymasterForm.value;
...
if(key!='visiblelist' && key!='hidelist'){
    if (this.bosubcategorymasterForm.controls[key] != null)
    {
        this.bosubcategorymasterservice.formData[key] = this.bosubcategorymasterForm.controls[key].value;
    }
}
}
}
console.log(this.bosubcategorymasterservice.formData);
this.bosubcategorymasterservice.formData=this.bosubcategorymasterForm.value;
this.bosubcategorymasterservice.saveOrUpdatebosubcategorymasters().subscribe(
async res => {
debugger;
...
{
this.dialogRef.close((res as any).result.value.bosubcategorymaster);
    return;
}
else
{
...
this.bosubcategorymasterservice.clearList();
if(bclear){
this.resetForm();
}
else{
