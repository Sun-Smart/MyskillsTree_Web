...
import { boreportService } from '../../../../../../n-tire-bo-app/src/app/service/boreport.service';
...
import { erpsuppliercertificationService } from '../../../../../../n-tire-procurement-app/src/app/service/erpsuppliercertification.service';
...
import { erppurchaserequestComponent } from '../../../../../../n-tire-procurement-app/src/app/pages/forms/erppurchaserequest/erppurchaserequest.component';
...
export class ReportViewerCtrlComponent implements OnInit, AfterViewInit {
    ...
    constructor(
        ...
        @Inject(DOCUMENT) private document, private elementRef: ElementRef, private datePipe: DatePipe
    ) {
...
*/
    ...
    getJsonObject(rowData,val)
    {
        if(rowData[val]!=undefined && rowData[val]!=null)
        {
            return JSON.parse(rowData[val]);
            /*
            let retval="";
            debugger;
    ...
    route(action, recordid = null) {
        ...
        switch (action) {
            case 'url':
                ...
                let url='/#/home/' + formname + '/' + formname + '/edit/' + encodeURIComponent(this.selecteddata['pkcol'])  + query;
            ...
            case 'view':
                ...
                break;
            case 'edit':
                this.router.navigate(['/home/' + formname + '/' + formname + '/edit/' + this.selecteddata['pkcol'] + query]);
    ...
    */
    ...
    async updatemenu(reportcode) {
        let res = await this.bomenumasterservice.getListBymenuurl(reportcode);
        console.log(res);
    ...
    opendialog1(action, rowData) {
        ...
        if(rowData["noneditable"]=='1')
            stredit='view';
        //let url = "/#/workflow/" + rowData["modulename"] + "/" + rowData["modulename"] + "/"+stredit+"/" + encodeURIComponent(rowData["pkvalue"]);
        let url = "/#/home/" + rowData["modulename"] + "/" + rowData["modulename"] + "/"+stredit+"/" + encodeURIComponent(rowData["pkvalue"]);
        this.sharedService.alert(url);
        this.dialog.open(dataComponent,
            {
    ...
    async Initialize(initalizeid, status1) {
        ...
        if (this.sharedService.menuid == undefined && this.currentRoute.snapshot.paramMap.get('id')) {
            ...
            if ((this.currentRoute.component as any).name.toLowerCase() == "boreportviewercomponent") {
                let res = await this.bomenumasterservice.getListBymenuurl(id);
                console.log(res);
                //this.sharedService.alert("res ");
...
//to see now
        if (false && this.pmenuid) {
            ...
            let res = await this.bomenumasterservice.getbomenumastersByID(parseInt(this.pmenuid));
            console.log("menuid");
            console.log(this.pmenuid);
            console.log(res);
        ...
        this.boreportservice.getboreportsList().then((data) => {
