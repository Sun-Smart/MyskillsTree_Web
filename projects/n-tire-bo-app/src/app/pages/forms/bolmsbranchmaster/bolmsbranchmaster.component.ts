...
@ViewChild('tblbobranchlocationssource',{static:false}) tblbobranchlocationssource: Ng2SmartTableComponent;
...
bobranchlocationsvisiblelist:any;
bobranchlocationshidelist:any;
...
DeletedbobranchlocationIDs: string="";
bobranchlocationsID: string = "2";
bobranchlocationsselectedindex:any;
DeletedbouserbranchaccessIDs: string="";
bouserbranchaccessesID: string = "3";
...
this.SetbobranchlocationsTableConfig();
  setTimeout(() => {
  this.SetbobranchlocationsTableddConfig();
  });
...
if (salesdirectorDetail.salesdirector && salesdirectorDetail) {
this.bobranchmasterForm.patchValue({
salesdirector: salesdirectorDetail.salesdirector,
...
this.bobranchmasterservice.bobranchlocations=[];
this.bobranchlocationsLoadTable();
this.bobranchmasterservice.bouserbranchaccesses=[];
this.bobranchmasterservice.Insertbouserbranchaccesses=[];
...
for (let i=0;i<this.bobranchmasterservice.bobranchlocations.length;i++) {
this.bobranchmasterservice.bobranchlocations[i].locationid=null;
}
for (let i=0;i<this.bobranchmasterservice.bouserbranchaccesses.length;i++) {
this.bobranchmasterservice.bouserbranchaccesses[i].accessid=null;
}
    }
...
this.bobranchlocationsvisiblelist=res.bobranchlocationsvisiblelist;
...
this.bobranchmasterservice.bobranchlocations = res.bobranchlocations;
this.SetbobranchlocationsTableConfig();
this.bobranchlocationsLoadTable();
  setTimeout(() => {
  this.SetbobranchlocationsTableddConfig();
  });
this.bobranchmasterservice.bouserbranchaccesses = res.bouserbranchaccesses;
this.SetbouserbranchaccessesTableConfig();
this.bouserbranchaccessesLoadTable();
  setTimeout(() => {
  this.SetbouserbranchaccessesTableddConfig();
...
this.bobranchmasterservice.formData.DeletedbobranchlocationIDs = this.DeletedbobranchlocationIDs;
this.bobranchmasterservice.formData.DeletedbouserbranchaccessIDs = this.DeletedbouserbranchaccessIDs;
...
if (this.bobranchlocationssource.data)
{
    for (let i = 0; i < this.bobranchlocationssource.data.length; i++)
    {
        if (this.bobranchlocationssource.data[i].fileattachmentlist)await this.sharedService.upload(this.bobranchlocationssource.data[i].fileattachmentlist);
    }
}
...
AddOrEditbobranchlocation(event:any,locationid:any, branchid:any) {
let add=false;
if(event==null)add=true;
this.dialog.open(bobranchlocationComponent, 
{
data:  {  showview:this.showview,save:false,event,locationid, branchid,visiblelist:this.bobranchlocationsvisiblelist,  hidelist:this.bobranchlocationshidelist,ScreenType:2  },
...
if(add)
{
this.bobranchlocationssource.add(res);
this.bobranchlocationssource.refresh();
}
else
{
this.bobranchlocationssource.update(event.data, res);
}
}
});
}

onDeletebobranchlocation(event:any,childID: number, i: number) {
if (childID != null)
this.DeletedbobranchlocationIDs += childID + ",";
this.bobranchmasterservice.bobranchlocations.splice(i, 1);
//this.updateGrandTotal();
}
...
bobranchlocationssettings:any;
bobranchlocationssource: any;

showbobranchlocationsCheckbox()
{
debugger;
if(this.tblbobranchlocationssource.settings['selectMode']== 'multi')this.tblbobranchlocationssource.settings['selectMode']= 'single';
else
this.tblbobranchlocationssource.settings['selectMode']= 'multi';
this.tblbobranchlocationssource.initGrid();
}
deletebobranchlocationsAll()
{
this.tblbobranchlocationssource.settings['selectMode'] = 'single';
}
showbobranchlocationsFilter()
{
  setTimeout(() => {
  this.SetbobranchlocationsTableddConfig();
  });
      if(this.tblbobranchlocationssource.settings!=null)this.tblbobranchlocationssource.settings['hideSubHeader'] =!this.tblbobranchlocationssource.settings['hideSubHeader'];
this.tblbobranchlocationssource.initGrid();
}
showbobranchlocationsInActive()
{
}
enablebobranchlocationsInActive()
{
}
async SetbobranchlocationsTableddConfig()
{
if(!this.bfilterPopulatebobranchlocations){

this.configservice.getList("locationcode").then(res=>
{
var datalocationcode2=res as any;
var defaultobj= { value:"", title:"Select..."};
this.databobranchlocationslocationcode3.push(defaultobj);
for(let i=0; i<datalocationcode2.length; i++){
var obj= { value: datalocationcode2[i].configkey, title: datalocationcode2[i].configtext};
this.databobranchlocationslocationcode3.push(obj);
}
var clone = this.sharedService.clone(this.tblbobranchlocationssource.settings);
if(clone.columns['locationcode']!=undefined)clone.columns['locationcode'].filter=    { type: 'list', config: {  selectText: 'Select...',list: JSON.parse(JSON.stringify(this.databobranchlocationslocationcode3)), }, };
if(clone.columns['locationcode']!=undefined)clone.columns['locationcode'].editor=    { type: 'list', config: {  selectText: 'Select...',list: JSON.parse(JSON.stringify(this.databobranchlocationslocationcode3)), }, };
this.tblbobranchlocationssource.settings =  clone;
this.tblbobranchlocationssource.initGrid();
});
}
this.bfilterPopulatebobranchlocations=true;
}
async bobranchlocationsbeforesave(event:any){
event.confirm.resolve(event.newData);



}
SetbobranchlocationsTableConfig()
{
this.bobranchlocationssettings = {
hideSubHeader: true,
mode: 'external',
selectMode: 'single',
actions: {
...
valuePrepareFunction: (cell,row) => {
var element= this.databobranchlocationslocationcode3.find(c=>c.value==cell);
...
bobranchlocationsLoadTable(){
if(this.ShowTableslist==null || this.ShowTableslist.length==0 || this.ShowTableslist.indexOf(this.bobranchlocationsID)>=0)
{
this.bobranchlocationssource=new LocalDataSource();
this.bobranchlocationssource.load(this.bobranchmasterservice.bobranchlocations as  any as LocalDataSource);
this.bobranchlocationssource.setPaging(1, 20, true);
}
}

//external to inline
/*
bobranchlocationsroute(event:any,action:any) {
switch ( action) {
case 'create':
if (this.bobranchmasterservice.bobranchlocations.length == 0)
{
    this.tblbobranchlocationssource.grid.createFormShown = true;
}
else
{
    let obj = new bobranchlocation();
    this.bobranchmasterservice.bobranchlocations.push(obj);
    this.bobranchlocationssource.refresh();
    if ((this.bobranchmasterservice.bobranchlocations.length / this.bobranchlocationssource.getPaging().perPage).toFixed(0) + 1 != this.bobranchlocationssource.getPaging().page)
    {
        this.bobranchlocationssource.setPage((this.bobranchmasterservice.bobranchlocations.length / this.bobranchlocationssource.getPaging().perPage).toFixed(0) + 1);
    }
    setTimeout(() => {
        this.tblbobranchlocationssource.grid.edit(this.tblbobranchlocationssource.grid.getLastRow());
    });
}
break;
case 'delete':
let index = this.bobranchlocationssource.data.indexOf(event.data);
this.onDeletebobranchlocation(event,event.data.locationid,((this.bobranchlocationssource.getPaging().page-1) *this.bobranchlocationssource.getPaging().perPage)+index);
this.bobranchlocationssource.refresh();
break;
}
}
...
switch ( action) {
case 'create':
this.AddOrEditbobranchlocation(event,null, this.formid);
break;
case 'view':
break;
case 'edit':
this.AddOrEditbobranchlocation(event,event.data.locationid,this.formid);
break;
case 'delete':
this.onDeletebobranchlocation(event,event.data.locationid,((this.bobranchlocationssource.getPaging().page-1) *this.bobranchlocationssource.getPaging().perPage)+event.index);
this.bobranchlocationssource.refresh();
break;
}
}
bobranchlocationsonDelete(obj) {
let locationid=obj.data.locationid;
if (confirm('Are you sure to delete this record ?')) {
this.bobranchmasterservice.deletebobranchmaster(locationid).then(res=>
this.bobranchlocationsLoadTable()
);
}
}
bobranchlocationsPaging(val)
{
debugger;
this.bobranchlocationssource.setPaging(1, val, true);
}

handlebobranchlocationsGridSelected(event:any) {
this.bobranchlocationsselectedindex=this.bobranchmasterservice.bobranchlocations.findIndex(i => i.locationid === event.data.locationid);
}
IsbobranchlocationsVisible()
{
if(this.ShowTableslist==null || this.ShowTableslist.length==0 || this.ShowTableslist.indexOf(this.bobranchlocationsID)>=0)
{
return "tbl smart-table-container";
...
this.bouserbranchaccessessource.load(this.bobranchmasterservice.bouserbranchaccesses as  any as LocalDataSource);
setTimeout(() => { 
...
{
this.bobranchmasterservice.Insertbouserbranchaccesses.push(row.data);
...
switch ( action) {
case 'create':
if (this.bobranchmasterservice.bouserbranchaccesses.length == 0)
{
...
else
{
    ...
    this.bobranchmasterservice.bouserbranchaccesses.push(obj);
    this.bouserbranchaccessessource.refresh();
    if ((this.bobranchmasterservice.bouserbranchaccesses.length / this.bouserbranchaccessessource.getPaging().perPage).toFixed(0) + 1 != this.bouserbranchaccessessource.getPaging().page)
    {
        this.bouserbranchaccessessource.setPage((this.bobranchmasterservice.bouserbranchaccesses.length / this.bouserbranchaccessessource.getPaging().perPage).toFixed(0) + 1);
    }
    setTimeout(() => {
...
this.bobranchmasterservice.Insertbouserbranchaccesses.push(obj as any);
}
else
{
